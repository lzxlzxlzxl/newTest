    ...
    void loopRead(BluetoothSocket socket) {
        ...
        try {
            if (!mSocket.isConnected())
                mSocket.connect();
            notifyUI(Listener.CONNECTED, mSocket.getRemoteDevice());   //通知UI获取获取套接字
            mOut = new DataOutputStream(mSocket.getOutputStream());     //获取输出流就是获取到的客户端发出的文件的输出流
            DataInputStream in = new DataInputStream(mSocket.getInputStream());   //获取输入流

           // writer=new OutputStreamWriter(mOut,"GBK");     //把输出流结合，包装成一成，后面输出可以直接输出字符串
        ...
        Util.EXECUTOR.execute(new Runnable() {
            ...
            public void run() {
                try {
                   ...
                   // Bitmap bitmap = HPRTPrinterHelper.INSTANCE.pdfToImage(new File(filePath), 0, 2336);
                   ...
                   // BluetoothManagerActivity.instance.init2();
                } catch (Throwable e) {
                    close();
                }
                isSending = false;
